...
const marker = '/* ==========================================================================\n   SR RESTAURANTS COMPREHENSIVE PRODUCTION UPGRADE STYLES\n   ========================================================================== */';
...
.topbar {
  ...
  border-radius: 18px;
  box-shadow: 0 8px 24px rgba(24, 52, 44, 0.07);
}
...
.restaurant-card-modern:hover .card-image {
...
.status-pill.open { background: #22c55e; color: #fff; }
.status-pill.closed { background: #ef4444; color: #fff; }
...
/* Forms (CreateRestaurant, Login, Register) */
.form-page-container,
.auth-page,
...
.profile-avatar {
  ...
  height: 76px;
  border-radius: 20px;
  background: linear-gradient(135deg, #18342c 0%, #2a5a4c 100%);
  color: #f3a23b;
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 2.2rem;
  font-weight: 800;
  box-shadow: 0 10px 24px rgba(24, 52, 44, 0.2);
}
...
.spinner-icon {
  font-size: 1.8rem;
  animation: food-spin-reverse 1s linear infinite;
}
...
\`;
  ...
  console.log('Successfully appended production upgrade styles to style.css!');
} else {
  console.log('Marker already exists in style.css.');
}
`;

fs.writeFileSync(path.join(__dirname, 'scripts/appendProductionStyles.js'), appendScript, 'utf8');
console.log('Created scripts/appendProductionStyles.js');
